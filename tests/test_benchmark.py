...
def test_benchmark_cost_returns_average(
    ...
    monkeypatch.setattr("bcrypt_speed_tests.benchmark.bcrypt.gensalt", lambda **_: b"salt")
    monkeypatch.setattr("bcrypt_speed_tests.benchmark.bcrypt.hashpw", fake_hashpw)

    timings = iter([0.0, 0.5, 0.5, 1.0, 1.0, 2.0])
    monkeypatch.setattr("bcrypt_speed_tests.benchmark.time.perf_counter", lambda: next(timings))
